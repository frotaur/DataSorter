...
class ViewFrameTrain(Frame):
    """
        Frame to display either a picture or a video, and live training of model behind the scenes.
    """
...
class ViewFrameClassic(Frame):
    """
        Frame to display either a picture or a video, and NO live training.
    """
    def __init__(self,fenetre,rawdatapath,datapath,**kwargs):
        ...
        self.vidpath = os.path.join(rawdatapath,"Videos")
        os.makedirs(self.vidpath,exist_ok=True)
        self.pairpath=os.path.join(datapath,"pairs","pairs.csv")
...
class ViewFrameSingle(Frame):
    """
        Frame to display a single picture or video. VERY HACK, VERY TEMPORARY
    """
    def __init__(self,fenetre,rawdatapath,datapath,**kwargs):
        Frame.__init__(self,fenetre,**kwargs)
        self.WIDTH = 960
        ...
        self.canvas = {'left':Canvas(self.canFrame['left'],width=self.CANVWIDTH*2,height=self.CANVHEIGHT),
                       'right':Canvas(self.canFrame['right'],width=0,height=0)}
...
# +++++++++++++++++++++++++++++++ LAYOUT ++++++++++++++++++++++++++++++++++++++++
        ...
        self.canFrame['left'].pack(side=LEFT, padx=5)
        self.canFrame['right'].pack(side=RIGHT, padx=5)
        self.canFrame['right'].pack_forget()
    ...
    def make_pair_data(self):
        ...
        for i in tqdm(range(len(self.all_data))) :
            data['left'].append(self.all_data[i])
            data['right'].append(self.all_data[i])
